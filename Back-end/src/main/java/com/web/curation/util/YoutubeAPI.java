	...
	public String search(String search) throws IOException {
		...
		for (JsonElement e : itemArray) {
			...
			if (searchService.isUrl(vno, search)) {
				...
				boardDto.setB_url("https://www.youtube.com/embed/"
						+ e.getAsJsonObject().get("id").getAsJsonObject().get("videoId").getAsString());
				...
				boardDto.setB_thumbnail(snippet.getAsJsonObject().get("thumbnails").getAsJsonObject().get("high")
						.getAsJsonObject().get("url").getAsString());
				boardDto.setB_date(
						str.substring(0, 4) + "년 " + str.substring(5, 7) + "월 " + str.substring(8, 10) + "일");
